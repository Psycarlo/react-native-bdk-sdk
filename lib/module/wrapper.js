...
import { ElectrumClient, EsploraClient, KyotoClient, RpcClient, TxBuilder, Wallet, createWallet as rawCreateWallet, createWalletFromMultipath as rawCreateWalletFromMultipath } from "./generated/bdk_ffi.js";
...
export class BdkKyotoClient {
  constructor(wallet, opts) {
    this.inner = new KyotoClient(wallet.raw, opts.scanType, opts.requiredPeers ?? 2, opts.peers ?? [], opts.dataDir, opts.handler);
  }
  get raw() {
    return this.inner;
  }
...
export class BdkRpcClient {
  constructor(opts) {
    const a = opts.auth;
    this.inner = new RpcClient(opts.url, a.type === 'userPass' ? a.username : undefined, a.type === 'userPass' ? a.password : undefined, a.type === 'cookieFile' ? a.path : undefined);
  }
  get raw() {
    return this.inner;
  }
...
export async function bdkCreateWalletFromMultipath(descriptor, network, dbPath) {
...
export class BdkWallet {
  ...
  static fromMultipath(descriptor, network, dbPath) {
    return BdkWallet.fromRaw(Wallet.newFromMultipath(descriptor, network, dbPath));
  }
  get raw() {
    return this.inner;
  }
