...
impl Wallet {
    ...
    pub fn tx_details(&self, txid: String) -> Result<Option<TxDetails>, BdkError> {
        let tid: Txid = txid.parse().map_err(|e| BdkError::InvalidTransaction {
    ...
    pub fn calculate_fee(&self, tx_hex: String) -> Result<u64, BdkError> {
        let tx = self.decode_tx(&tx_hex)?;
        let w = self.inner.lock().unwrap();
        let fee = w.calculate_fee(&tx).map_err(|e| BdkError::CalculateFeeError {
            message: e.to_string(),
        })?;
        Ok(fee.to_sat())
    }
    ...
    pub fn calculate_fee_rate(&self, tx_hex: String) -> Result<f64, BdkError> {
        ...
        let rate = w
            ...
            .map_err(|e| BdkError::CalculateFeeError {
                message: e.to_string(),
            })?;
    ...
    pub async fn full_scan_with_electrum(
        &self,
    ...
    pub async fn sync_with_electrum(
        &self,
        client: Arc<ElectrumClient>,
        _stop_gap: u64,
    ) -> Result<(), BdkError> {
    ...
    pub fn policies(&self, keychain: KeychainKind) -> Result<Option<String>, BdkError> {
        ...
        let policy = w
            ...
            .map_err(|e| BdkError::InvalidDescriptor {
                message: format!("Failed to compute policies: {}", e),
            })?;
    ...
    fn persist_wallet(
        ...
        wallet
            .persist(&mut *conn)
            .map_err(|e| BdkError::PersistError {
                message: format!("Failed to persist wallet: {}", e),
            })
    }
    ...
    fn decode_tx(&self, tx_hex: &str) -> Result<Transaction, BdkError> {
        ...
        consensus::encode::deserialize(&bytes).map_err(|e| BdkError::InvalidTransaction {
    ...
    fn build_send_tx(
        ...
        let addr = address
            ...
            .map_err(|e| BdkError::InvalidAddress {
                message: format!("Cannot parse address '{}': {}", address, e),
            })?
            .require_network(network)
            .map_err(|e| BdkError::InvalidAddress {
                message: format!("Address '{}' not valid for network {:?}: {}", address, network, e),
            })?;
        ...
        psbt.extract_tx().map_err(|e| BdkError::InvalidPsbt {
            message: format!("Failed to extract tx: {}", e),
        })
    }
